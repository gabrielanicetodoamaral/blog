...
eval('mysqli_stmt_bind_param($stmt, \'' . implode('', $tipo) . '\', ' . implode(', ', $vars) . ');');
...
function atualiza(string $entidade, array $dados, array $criterio = []) : bool
{
    $retorno = false;
    $coringa_dados = [];
    $coringa_criterio = [];  // Inicializar para evitar undefined variable
    ...
    foreach ($criterio as $expressao) {
        ...
        $$nome_campo = $dado;
        $coringa_criterio[] = $expressao;  // Preenchendo o array coringa_criterio
    }
    ...
    if(isset($tipo)){
        ...
        $comando .= ', $' . implode(', $', array_merge(array_keys($dados), $campos_criterio));
        $comando .= ');';
